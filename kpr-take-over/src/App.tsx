...
export default function App() {
  ...
  const handleShare = async () => {
    if (!sim.result || !shareRef.current || sharing) return;
    setSharing(true);
    try {
      ...
      const file = new File([blob], 'simulasi-kpr.png', { type: 'image/png' });
      const teks =
        `Simulasi KPR Take Over — ${sim.result.hemat ? 'hemat' : 'selisih'} ${formatRingkas(Math.abs(sim.result.selisih))} ` +
        `(${formatPersenLabel(Math.abs(sim.result.selisihPersen))}). Tanpa take over ${formatRingkas(sim.result.totalTanpaTakeOver)} ` +
        `vs dengan take over ${formatRingkas(sim.result.totalDenganTakeOver)}. Dihitung dengan Kev's KPR Calculator.`;
      const nav = navigator as Navigator & { canShare?: (d: ShareData) => boolean };
      if (nav.canShare && nav.canShare({ files: [file] })) {
        await nav.share({ files: [file], title: 'Simulasi KPR Take Over', text: teks });
      } else {
        ...
        showToast('Gambar diunduh — lampirkan ke WhatsApp');
      }
    } catch (e) {
  ...
  return (
    <div className="app">
      ...
      <div aria-hidden style={{ position: 'fixed', left: -10000, top: 0, pointerEvents: 'none', opacity: 0 }}>
